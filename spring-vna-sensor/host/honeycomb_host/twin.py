...
MOD_Q = 12   # 调制表定点小数位: 单位 LSB/4096 (int32)


def mod_table_from_phasors(V: np.ndarray, Ich: np.ndarray) -> np.ndarray:
    """固件 v0.2 驻留调制表 (DBG src 3): 每驻留 int32 [Vi, Vq, Ii, Iq], 单位 LSB/4096 (MOD_Q=12), 采样 v_n = Vi·cos + Vq·sin + 均匀亚 LSB 抖动,
    使主机 decode_dwells 得到的复电压 == 输入相量 (Vi = Re/LSB·4096, Vq = −Im/LSB·4096, 与 accumulate 同约定). |V| > 2047 LSB 在固件饱和."""
    V = np.asarray(V, complex); Ich = np.asarray(Ich, complex)
    q = 1 << MOD_Q
    lim = 2 ** 31 - 1
    t = np.zeros((len(V), 4), np.int32)
    t[:, 0] = np.clip(np.rint(np.real(V) / LSB * q), -lim, lim); t[:, 1] = np.clip(np.rint(-np.imag(V) / LSB * q), -lim, lim)
    t[:, 2] = np.clip(np.rint(np.real(Ich) / LSB * q), -lim, lim); t[:, 3] = np.clip(np.rint(-np.imag(Ich) / LSB * q), -lim, lim)
    return t
