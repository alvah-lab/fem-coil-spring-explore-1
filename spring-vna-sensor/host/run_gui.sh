...
#   末尾可加 --log 开会话日志, 例: ./run_gui.sh twin --log
...
case "${1:-twin}" in
  twin)   exec python -m honeycomb_host.gui.main --source twin --scene "$SCENE" --noise "$NOISE" "${@:2}" ;;
  udp)    python -m honeycomb_host.sim_device --scene "$SCENE" --noise "$NOISE" --drop "${DROP:-0}" &
          DEV=$!; trap "kill $DEV 2>/dev/null" EXIT; sleep 0.5
          python -m honeycomb_host.gui.main --source udp --device 127.0.0.1:5000 "${@:2}" ;;
  board)  exec python -m honeycomb_host.gui.main --source udp --device "${DEVICE:-192.168.2.128:5000}" "${@:2}" ;;
